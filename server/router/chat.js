...
router.get("/all-active-conversations", middleware, async (req, resp) => {
    ...
    try {
        /*const activeChats = await Chat.find({
            users: {
                $elemMatch: { userType: { $in: ["customer", "admin"] }, userId: req.accountId }
            }
        }).populate({
            path: "users.userId", 
            model: req.userType, 
        ...
        const activeChats = await Chat.find({
            ...
            path: "users.userId",
            model: req.userType, // Replace with your actual model name for users
            select: "name email",
        });
    ...
    try {
        ...
        const isChat = await Chat.findOne({
            users: {
                $all: [
                    { $elemMatch: { userType: { $in: ["customer", "admin"] }, userId: req.accountId } },
                    { $elemMatch: { userType: { $in: ["customer", "admin"] }, userId: receiverId } }
                ]
            }
        });
        // chat exixt
