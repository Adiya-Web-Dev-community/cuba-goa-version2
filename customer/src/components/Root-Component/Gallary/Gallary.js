    ...
    try {
      ...
      if (response.data.success) {
        ...
        for (let i = 0; i < response.data.resort[0]?.rooms.length; i++) {
          for (
            let j = 0;
            j < response.data.resort[0]?.rooms[i].imgUrl.length;
            j++
          ) {
  ...
  return (
    <>
      <div className="gallery-wrap">
        <div className="header-navbar">
          {allProperties.map((resort, i) => {
            return (
              <div key={i + 1}>
                <p
                  onClick={() => handleGetImages(resort._id, resort.resortName)}
                  style={{ color: "black" }}
                >
                  {resort.resortName}
                </p>
        ...
        <p
          style={{ marginLeft: "3rem", color: "lightgrey", marginTop: "3rem" }}
        >
