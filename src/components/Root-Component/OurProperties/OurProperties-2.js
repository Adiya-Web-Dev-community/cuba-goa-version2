...
const OurProperties = () => {
  ...
  function deleteRoom(id) {
    ...
    const obj = ourPropertiesData.find(
      (el) => el.resortName === selectedValue[0]
    );
    // const newObject = obj?.rooms?.filter((el) => el._id !== id)
    // obj.rooms = newObject
    axios.put(`http://localhost:4001/hotelbook/${obj._id}`).then((resp) => {
      resp
        .json()
        .then(() => {
          alert("successfully submitted");
      ...
      <main className="our-properties-main">
        <div className="quba-goa-search">
          ...
          <div className="about-hotel">
            {ourPropertiesData
              .filter((el) => el.resortName === selectedValue[0])
              .map((el) => (
                <>
            ...
            {ourPropertiesData[0] && (
              <div className="overview-of-rooms">
                <div className="room-resortName">
                  <h2>
                    {" "}
                    {ourPropertiesData.find(
                      (el) => el.resortName === selectedValue[0]
                    )?.resortName + " ROOMS"}
                  </h2>
                </div>
